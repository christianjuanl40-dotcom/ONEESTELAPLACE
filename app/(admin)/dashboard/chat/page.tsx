...
export default function AdminSupportChatPage() {
  ...
  const { messages, sendMessage, markAsRead, loadChat, unloadChat } = useChat()
  ...
  useEffect(() => {
    if (!user || (user.role === "staff" && !user.permissions?.chat)) return
    console.log("[Firestore Listener START] UsersPresence")
    const usersQuery = query(collection(db, "users"), where("role", "==", "client"), limit(500))
  ...
  }, [user?.role, user?.permissions?.chat])
